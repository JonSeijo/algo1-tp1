...
%\aux{distanciaHastaObstaculoOMargen}{d : Drone, s : Sistema}{\int}{if(obstaculoEnFilaAIzquierda(d, s))\newline
%\hspace{1mm}then(prm(obstaculoDelante[|obstaculoDelante|-1]) - prm(posicionActual(d)))\newline
%\hspace{1mm}else(prm(posicionActual(d)-0))}

%aux del 24
<<<<<<< HEAD
\aux{sinCensar}{s: Sistema, sis: Sistema, d: Drone}{\bool}{(\exists \selector{x} enjambreDrones(s))\hspace{1mm} id(x) == id(d)  \land \newline contenido(campo(s), prm(posicionActual(x)), sgd(posicionActual(x))) == Cultivo \land \newline estadoCultivo(sis), prm(posicionActual(x)), sgd(posActual(x)) == NoCensado }
=======
\aux{sinCensar}{s: Sistema, sis: Sistema, d: Drone}{\bool}{(\exists \selector{x} enjambreDrones(s)) id(x) == id(d)  \land \newline contenido(campo(s), prm(posicionActual(x)), sgd(posicionActual(x))) == Cultivo \land \newline estadoCultivo(sis), prm(posicionActual(x)), sgd(posActual(x)) == NoCensado }

>>>>>>> ff11d2b1798e646eb6580625654ed4c46939e029
