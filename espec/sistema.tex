...
((estadoCultivo(pre(s),i,j) == RecienSembrado) \implica (estadoCultivo(s, i, j) == EnCrecimiento))\newline
\land ((estadoCultivo(pre(s),i,j) == EnCrecimiento) \implica (estadoCultivo(s, i, j) == ListoParaCosechar))\newline
\land ((estadoCultivo(pre(s), i, j) \neq RecienSembrado\hspace{1mm} \land \hspace{1mm} estadoCultivo(pre(s), i, j)  \neq EnCrecimiento) \newline \implica estadoCultivo(pre(s), i, j) == estadoCultivo(s, i, j))}
...
estadoCultivo(pre(s), i ,j) == estadoDelCultivo(s, i, j)}
